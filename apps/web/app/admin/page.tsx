...
import { getScriptTagLabel } from "@/lib/script-tags"
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string; invited?: string }> }) {
  ...
  return (
    <div className="grid gap-8">
      ...
      <section>
        <div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-semibold">Scripts</h2><p className="text-sm text-muted-foreground">{scripts.length} total scripts</p></div><Link href="/admin/scripts/new" className={buttonVariants()}><IconPlus /> New script</Link></div>
        <Card className="overflow-hidden p-0"><Table><TableHeader><TableRow><TableHead>Script</TableHead><TableHead>Status</TableHead><TableHead>Tags</TableHead><TableHead>Stats</TableHead><TableHead>Updated</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{scripts.map((script) => <TableRow key={script.id}><TableCell><p className="font-medium">{script.title}</p><p className="text-xs text-muted-foreground">/{script.slug}</p></TableCell><TableCell><Badge variant={script.published ? "default" : "secondary"}>{script.published ? "Published" : "Draft"}</Badge></TableCell><TableCell><div className="flex flex-wrap gap-1">{script.tags.length ? script.tags.map((tag) => <Badge key={tag} variant="outline">{getScriptTagLabel(tag)}</Badge>) : <span className="text-xs text-muted-foreground">None</span>}</div></TableCell><TableCell className="text-xs text-muted-foreground">{formatStatCount(script.viewCount)} views · {formatStatCount(script.copyCount)} copies · {formatStatCount(script.downloadCount)} downloads</TableCell><TableCell className="text-muted-foreground">{script.updatedAt.toLocaleDateString()}</TableCell><TableCell><div className="flex justify-end gap-1"><Link href={`/admin/scripts/${script.id}/edit`} aria-label={`Edit ${script.title}`} className={buttonVariants({ variant: "ghost", size: "icon" })}><IconEdit /></Link><DeleteScriptButton id={script.id} title={script.title} /></div></TableCell></TableRow>)}</TableBody></Table></Card>
      </section>
